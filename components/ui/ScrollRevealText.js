...
export default function ScrollRevealText(props) {
  ...
  const presetName = props.preset || "Soft Words";
  const preset = presetName !== "Custom" ? PRESETS[presetName] : null;

  const {
    children,
    text: textProp,
    colorHidden = "rgba(0, 0, 0, 0.15)",
  ...
  const Tag = htmlTag || as || tag || "span";

  const text = textProp || (typeof children === "string" ? children : String(children || ""));
  const splitMode = preset?.splitMode ?? props.splitMode ?? "Words";
  const revealDirection = preset?.revealDirection ?? props.revealDirection ?? "Left to Right";
  const stagger = preset?.stagger ?? props.stagger ?? 0.2;
  const xOffset = preset?.xOffset ?? props.xOffset ?? 0;
  const yOffset = preset?.yOffset ?? props.yOffset ?? 0;
  const blur = preset?.blur ?? props.blur ?? 8;
  ...
  if (isLinesMode && lineGroups) {
    return (
      <Tag ref={containerRef} className={className} style={containerStyle}>
